...
def _read_tsarray(base_dir, var, tstore_structure):
    ...
    ts_fpaths, identifiers = _get_ts_info(
        base_dir=base_dir, var=var, tstore_structure=tstore_structure
    )
    # Read TS objects
...
def _read_tsarrays(base_dir, metadata):
    ...
    list_ts_series = [
        _read_tsarray(base_dir=base_dir, var=var, tstore_structure=tstore_structure)
