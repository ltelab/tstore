...
def _get_tsarray_columns(df): 
    ...
    tsarray_columns = columns[[isinstance(df[column].dtype, TSDtype) for column in columns]].tolist()
...
def _write_ts_series(ts_series, base_dir, tstore_structure):
    ...
    for identifier, ts in zip(identifiers, ts_series): 
        if ts: # not empty 
            ts_fpath = define_tsarray_filepath(base_dir=base_dir, 
...
def _write_tsarrays(df, base_dir, tstore_structure):
    ...
    for column in tsarray_columns:
        _write_ts_series(ts_series=df[column], 
                         base_dir=base_dir, 
                         tstore_structure=tstore_structure)
...
def _write_metadata(df, base_dir, tstore_structure):
    ...
    write_metadata(base_dir=base_dir, 
                   ts_variables=ts_variables, 
                   tstore_structure=tstore_structure)
    

def write_tstore(df, base_dir, 
                 partition_str=None, 
    ...
    # Write TSArrays 
    _write_tsarrays(df, base_dir=base_dir, 
                    tstore_structure=tstore_structure)                  
